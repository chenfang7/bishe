      ...
      <div class="survey" style="margin:0px auto;">
       <div class="surveyhead" id="ctl00_ContentPlaceHolder1_JQ1_divHead" style="border: 0px;">
        <h1 id="ctl00_ContentPlaceHolder1_JQ1_h1Name" style="position:relative;">
         <span id="ctl00_ContentPlaceHolder1_JQ1_lblQuestionnaireName">分析与下载</span>
        </h1>
        <button class="button button-3d button-action button-pill button-tiny" style="float:right;" onclick="download(${qaa[0].qid})">问卷结果下载</button>
       ...
       <div class="surveycontent" id="ctl00_ContentPlaceHolder1_JQ1_question">
        <div id="ctl00_ContentPlaceHolder1_JQ1_surveyContent">
         <fieldset class="fieldset" id="fieldset1">
             ...
             <c:forEach items="${qaa}" var="q" varStatus="stat">
             <!-- 单选 -->
             <c:if test="${q.type=='1'||q.type=='2'}">
           
          <div class="div_title_question_all">
            <div class="div_topic_question">${stat.count}.</div>
            <div class="div_title_question" id="divTitle1">${q.tname}<span style="color:gainsboro;">&nbsp;<c:if test="${q.type=='1'}">[单选题]</c:if><c:if test="${q.type=='2'}">[多选题]</c:if></span></div>
            <div style="clear:both;"></div>
           </div>
           <div class="div_table_radio_question" id="divquestion3">
            ...
            <ul class="ulradiocheck">
													<table
														...
														<tbody>
															<c:forEach items="${q.datas}" var="data" varStatus="stata">
																<tr>
																	<td>${stata.count}</td>
																	<td>${data.answer}</td>
																	<td>${data.sum}</td>
					...
					<div id="main${stat.count}" style="height:200px;"></div>
	<script>
		// 绘制图表。
		var arr =new Array();
		<c:forEach items="${q.datas}" var="data" varStatus="stata">
			var obj =new Object();
			obj.name='${data.answer}'
			obj.value='${data.sum}'
			arr.push(obj)
		</c:forEach>
		echarts.init(document.getElementById('main${stat.count}')).setOption({
			series : {
				type : 'pie',
				data : arr
			}
          ...
          <c:if test="${q.type=='3'}">
           ...
           <div class="div_title_question_all">
            <div class="div_topic_question">${stat.count}.</div>
            <div class="div_title_question" id="divTitle1">${q.tname}<span style="color:gainsboro;">&nbsp;[填空题]</span></div>
            <div style="clear:both;"></div>
           </div>
           <div class="div_table_radio_question" id="divquestion3">
            <div class="div_table_clear_top"></div>
